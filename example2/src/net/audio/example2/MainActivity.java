...
//	String toIP = "192.168.1.7";
	 String toIP = "192.168.1.90";
				...
				try {
					...
					while (isRecording) {
						...
						while (outputBufferIndex >= 0) {
							...
							// Log.d("AudioEncoder", outData.length +
							// " bytes encoded");
							// -------------
							packet = new DatagramPacket(outData, outData.length, InetAddress.getByName(toIP), localPort);
							ds.send(packet);
							// ------------
				...
				try {
					...
					while (isPlaying) {
						try {
							...
							// Log.d("UDP Receiver"," received !!! from " +
							// address);

							data = new byte[packet.getLength()];
							System.arraycopy(packet.getData(), packet.getOffset(), data, 0, packet.getLength());

							// Log.d("UDP Receiver", data.length +
							// " bytes received");
							...
							while (outputBufferIndex >= 0) {
								...
								// Log.d("AudioDecoder", outData.length +
								// " bytes decoded");
